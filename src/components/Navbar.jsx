...
export default function Navbar() {
    ...
    const handleContactClick = () => {
        setIsOpen(false);
        // If we are on home page ('/') OR any project page containing 'project', try to scroll to contact
        // Assuming project pages are like /project/:slug or similar, and they HAVE the contact form at bottom

        const contactSection = document.getElementById('contact');
        if (contactSection) {
            contactSection.scrollIntoView({ behavior: 'smooth' });
        } else {
            // If contact section not found on current page, go to home's contact section
            navigate('/#contact');
        }
    };
    ...
    const navItems = [
        { name: 'Home', href: '#' },
        { name: 'Projects', href: '/' }, // The "Product Page" / Main Page
        { name: 'Services', href: '#' },
        { name: 'About Us', href: '#' }
    ];
            ...
            <nav className="fixed top-6 left-1/2 -translate-x-1/2 z-50 w-full max-w-6xl 2xl:max-w-7xl px-4">
                <div className="w-full flex items-center justify-between px-6 md:px-8 py-3 md:py-4 bg-white/70 backdrop-blur-xl border border-white/40 rounded-full shadow-[0_8px_30px_rgb(0,0,0,0.04)] transition-all hover:bg-white/90 relative z-50">
                    ...
                    <div className="hidden md:block">
                        <button onClick={handleContactClick} className="px-6 py-2.5 bg-transparent border border-[#FF3B30] text-[#FF3B30] text-sm font-semibold rounded-full hover:bg-[#FF3B30] hover:text-white transition-all shadow-sm">
                            Contact us
                        </button>
                    </div>
                    ...
                    >
                        ...
                        >
                            ...
                            <motion.div variants={itemVariants} className="pt-8">
                                <button onClick={handleContactClick} className="px-8 py-4 bg-gray-900 text-white text-lg font-medium rounded-full hover:bg-[#FF3B30] transition-colors shadow-xl">
                                    Contact us
                                </button>
                            </motion.div>
                        </motion.div>
